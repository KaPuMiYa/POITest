...
public class ImportExcel {
	...
	public static void readExcel(File file, String province) {
		try {
			...
			e.printStackTrace();
			System.out.println("excel异常：" + file.getAbsolutePath());
			System.out.println(e);
		}
	...
	public static void save(String province, String title) {
		if (title.contains("城市综合体") || title.contains("文物古建筑")) {
			...
			specialEquipment(); // 单独遍历特种装备，获取对应省份
		}
	}
	...
	public static void sysPrint() {
		...
		String line = System.getProperty("line.separator"); // 打印到文件中自动换行
	...
	public static void writeExcel(String path) {
		...
		try {
			...
			for (int i = 0; i < sheetNumber; i++) {
				...
				File newFile = new File("D:\\北京项目\\数据库表\\" + "各总队基础数据采集情况统计表" + time + ".xls");
...
//		try {
//			System.setOut(new PrintStream(new FileOutputStream("D:\\log3.txt")));
...
//		}
		...
		transMap(); // 将各种分类转换为对应的数字
		String path = "D:\\北京项目\\数据库表\\模板.xls";
